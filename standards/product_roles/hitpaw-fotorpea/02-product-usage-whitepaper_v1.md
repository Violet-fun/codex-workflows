...
FotorPea 适用于图片任务：低清或模糊图片增强、图片放大、噪点／压缩／暗光／文字改善、旧照片修复和上色、背景移除、图片对象移除、AI 图片生成与编辑。只有当它直接解决文章的图片主任务，才作为 `formal_method`；短小的相关单图补充使用 `ultra_tip`，有独立后续决策价值的相关模块使用 `related_supplement`，两者均须通过 `16` 的相邻性判定。
