...
- `ultra_tip`：主任务完成后的短小、可操作相邻需求补充，清楚说明不适用范围。
- `related_supplement`：具有独立读者决策价值的相关后续模块；位于核心答案后，不伪装成主方法、榜单结论或 CTA。
- `excluded`：无关、证据不足、版本不匹配或有误导风险；不出现在文章中。普通推荐产品的范围内排除不阻断主文章；详细分流见 `16`。
