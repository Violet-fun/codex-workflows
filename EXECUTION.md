...
4. 按当前阶段读取相关详细标准，不把无关产品和类型规则全部加载。任务有推荐产品时执行 `16-产品相邻性与阻断分流_v1.md` 的五项判定；先给出简短启动回执，无**发布 P0**阻断项直接继续。
...
- 最终发布检查必须验证当前状态、实际文件和最终渲染记录。发布 P0 的 Fail/Hold/未执行均阻断 Final。产品 `research_incomplete` 或 `scoped_exclusion` 只阻断该产品进入生产文案，不能阻断已证实的主文章；已排除的候选 Claim 必须证明未进入生产文案。只有 `mandatory_product_inclusion: true` 与实时证据冲突时，产品露出进入 `blocked_waiting_for_user`。
